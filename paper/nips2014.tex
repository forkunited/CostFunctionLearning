...
Table~\ref{accuraciesTable} shows the micro-averaged accuracies on the Reuters
and 20 newsgroups tasks for the SVM baseline model and versions of SVMCLN with
different choices of normalization constants $\mathbf{n}$ and incorrect prediction
classes $\mathcal{S}$.  None of the SVMCLN variations improve the accuracy over
the SVM on the Reuters task, but inspection of the confusion matrix for the SVM
baseline provides some intuition for this lack of improvement.  The matrix shows
...
In contrast, every SVMCLN variation appears to give significantly improved accuracy 
over the SVM on the 20 Newsgroups data.  


%- Performance
%	- SVM baseline and cost learning
%	- Unordered performs at least as well as ordered
%	- Logical and expected n perform better than no normalization
%     - This is what we expected since no normalization won't properly learn costs
%
%- Learned cost weights for each model, relation to hierarchies

% - Measured accuracies on hierarchy
